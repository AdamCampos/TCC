...
fprintf('Impedância resistiva do motor = %f\n', Resistance)
fprintf('Pot Máxima de Saída %f @ Corrente = %f, Torque = %f\n', orangelineY(maxPwrIndex), redlineY(maxPwrIndex), orangelineX(maxPwrIndex))
fprintf('Eficiência Máxima %f @ Corrente = %f, Torque = %f\n', greenlineY(maxEffIndex), redlineY(maxEffIndex), orangelineX(maxEffIndex))
...
title('Pot Saída');
legend('Pot Saída');
ylabel('Pot Saída (Velocidade*Torque)');
xlabel('Torque (kgf.cm)');
subplot(dim1,dim2,4);
grid on;
grid minor;
...
title('Eficiência');
legend('Eficiência');
ylabel('Eficiencia (Pot. Saída/Pot Entrada)');
xlabel('Torque (kgf.cm)');
