...
#define vozDesce_      (4)
#define vozSobe_       (5)
#define vozReserva1    (6)
#define vozReserva2    (7)

Adafruit_TFTLCD tft(LCD_CS, LCD_CD, LCD_WR, LCD_RD, LCD_RESET);
TouchScreen ts = TouchScreen(XP, YP, XM, YM, 600);
VR myVR(50, 51);   //(No Mega RX=50, TX=51, no Uno usar 2 e 3)
uint8_t buf[64];
...
void setup() {
  ...
  /*-------------------------------------------------------------------------------------------------------------------------------------*/
  //Inicia comunicacao com porta serial 0 e espera a conexao acontecer (comunicacao com o programador - pc)
  Serial.begin(9600);
  while (!Serial) {}
  //Inicia comunicacao com porta serial 1 e espera a conexao acontecer (comunicacao com o controlador dos motores)
  Serial1.begin(9600);
  while (!Serial1) {}
  ...
  if (myVR.load((uint8_t)vozSobe_) >= 0) {
    Serial.println("vozSobe_ carregado");
  }
  if (myVR.load((uint8_t)vozDesce_) >= 0) {
    Serial.println("vozDesce_ carregado");
  }
...
void loop() {
  ...
  if(comando!=""){
  Serial.println("Enviando para o Nano comando de: " + comando);
  ...
  //  }
  //
  //  //Desenha a seta descendo ate a cabine se posicionar no primeiro piso
  //  else if (descida) {
  //    setaBaixo();
  //    subida = true;
  //    descida = false;
